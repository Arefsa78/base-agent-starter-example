...
bool Bhv_BasicOffensiveKick::shoot2(PlayerAgent *agent) {
    const WorldModel &wm = agent->world();
    const ServerParam &SP = ServerParam::i();

    Vector2D center_goal = Vector2D(SP.pitchHalfLength(), 0);
    if (wm.ball().pos().dist(center_goal) > 25)
        return false;

    for (double y = -6.5; y <= 6.5; y += 0.5) {
        Vector2D target(SP.pitchHalfLength(), y);

        dlog.addCircle(Logger::SHOOT, target, 0.125, "FF0000", true);

        Vector2D ball_pos = wm.ball().pos();
        Vector2D ball_vel = Vector2D::polar2vector(SP.ballSpeedMax(),
                                                   (target - ball_pos).th());
        int cycle = 3;
        ...
        while (cycle < 40) {
            ...
            for (int i = 1; i <= 11; i++) {
                ...
                int n_dash = opp->playerTypePtr()->cyclesToReachDistance(dist);

                int n_turn = 0;
                double opp_speed = opp->vel().r();
                double angle_diff = (opp->body() - (ball_pos - opp->pos()).th()).abs();
                while ( angle_diff > 15. ){
                    angle_diff -= opp->playerTypePtr()->effectiveTurn(SP.maxMoment(),
                                                                      opp_speed);
                    opp_speed *= opp->playerTypePtr()->playerDecay();
                    ++n_turn;
                }
                int opp_reach_cycle = n_dash + n_turn;
                if (opp_reach_cycle < cycle){
                    intercept = true;
                    break;
                }
            }
        ...
        if(!intercept){
            dlog.addLine(Logger::SHOOT, wm.ball().pos(), target, "#000000");
            Body_SmartKick(target, 3, 0.5, 3).execute(agent);
            return true;
        }
    }
    return false;
