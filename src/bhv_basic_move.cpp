...
bool
Bhv_BasicMove::block(PlayerAgent *agent) {
    const WorldModel& wm = agent->world();
    const ServerParam& SP = ServerParam::i();

    const AbstractPlayerObject* opp = wm.interceptTable()->fastestOpponent();
    if (opp == NULL || opp->unum() < 1)
        return false;

    // TODO WHY *0.7
    Vector2D opp_target = Vector2D(-SP.pitchHalfLength(), 0);
    Vector2D opp_vel = Vector2D::polar2vector(opp->playerTypePtr()->playerSpeedMax() * 0.7,
                                              (opp_target - opp->pos()).th());
    Vector2D opp_pos = opp->pos();

    int cycle = 1;
    while (cycle < 40){
        opp_pos += opp_vel;
        dlog.addCircle(Logger::BLOCK, opp_pos, 0.25, "FF0000", true);
        if (opp_pos.absX() > SP.pitchHalfLength())
            break;

        int reach = wm.self().playerType().cyclesToReachDistance(opp_pos.dist(wm.self().pos()));
        if (reach < cycle){
            dlog.addLine(Logger::BLOCK, wm.self().pos(), opp_pos, "000000");
            Body_GoToPoint(opp_pos, 0.5, 100).execute(agent);
            return true;
        }
        cycle++;
    }
...
bool
Bhv_BasicMove::block2(PlayerAgent *agent) {
    const WorldModel &wm = agent->world();
    const ServerParam &SP = ServerParam::i();
    ...
    while (cycle < 40) {
        ...
        for (int i = 1; i <= 11; i++) {
            ...
            if (tm == NULL || tm->unum() < 1)
                continue;
            int reach =tm->playerTypePtr()->cyclesToReachDistance(opp_pos.dist(tm->pos()));
            if (reach < cycle) {
                if (tm->unum() == wm.self().unum()) {
                    dlog.addLine(Logger::BLOCK, tm->pos(), opp_pos, "000000");
                    Body_GoToPoint(opp_pos, 0.5, 100).execute(agent);
                    return true;
                } else {
                    return false;
